...
const AddPost = () => {
  ...
  const validateForm = () => {
    let formErrors = {};
    if (!title) formErrors.title = t("NameRequired");
    if (!category) formErrors.categoryId = t("Category") + " " + t("NameRequired");
    if (!images || images.length === 0) {
      formErrors.imageuser = t("ImageRequired") + " " ;
    }
  ...
  const handleSave = () => {
    ...
    const request = edit
      ? axios.put(`https://app.15may.club/api/admin/posts/${sendData}`, newUser, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
    ...
    request
      .then(() => {
        toast.success(t("Posts") + " " + t(edit ? "CategoryUpdated" : "CategoryAdded"));
        setTimeout(() => {
    ...
    <div className=" mt-5">
      ...
      <div className="flex gap-5 px-2 ">
        ...
        <span className="text-3xl font-medium text-center text-four ">
          {t("posts")} /<span className="text-one"> {t(edit ? "Edit" : "Add")} </span>
        </span>
      </div>
      ...
      <div className="flex mt-6">
        <button
          ...
          onClick={handleSave}
        >
          {checkLoading ? t("Loading") : <span>{t(edit ? "Edit" : "Add")}</span>}
        </button>
      </div>
    </div>
  );
